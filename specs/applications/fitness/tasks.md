...
- [ ] T1389 [US8] Add `category` field to Habit entity (`health|fitness|productivity|learning|financial|social|custom`) + EF Core migration - 2h
- [ ] T1390 [US8] Add `linkedEventPattern`, `linkedEventIds`, `autoCompleteSource` fields to Habit entity + migration update - 2h
- [ ] T1391 [US8] Implement `HabitEventLinkService` — match incoming `EventCompleted` events against habit patterns/IDs and mark habit complete - 4h
- [ ] T1392 [US8] Subscribe Fitness service to `events/event.completed` on the event bus; trigger bidirectional sync with `events/habit.completed` publish - 3h
- [ ] T1393 [US8] Extend `HabitService.CompleteHabit` to publish `HabitCompleted` event (bidirectional: also marks linked task done) - 2h
- [ ] T1394 [US8] Add `GET /api/v1/fitness/habits/categories` endpoint returning category values with completion counts - 2h
- [ ] T1395 [US8] Add `POST /api/v1/fitness/habits/:id/link-event` and `DELETE /api/v1/fitness/habits/:id/link-event/:eventId` endpoints - 2h
- [ ] T1396 [US8] Update habit DTOs to include `category`, `linkedEventPattern`, `linkedEventIds`, `autoCompleteSource` - 1h
- [ ] T1397 [US8] Write unit tests for HabitEventLinkService and bidirectional sync logic (12+ tests) - 3h
- [ ] T1398 [US8] Create `category` field in `Habit` TypeScript interface (`apps/web/src/types/fitness.ts`) - 1h
- [ ] T1399 [US8] Add category selector (pill group / dropdown) to HabitForm component - 2h
- [ ] T1400 [US8] Add category filter bar to HabitsDashboard with per-category aggregate stats - 3h
- [ ] T1401 [US8] Add event-linking section to HabitForm (pattern input + explicit event picker) - 3h
- [ ] T1402 [US8] Show event-link indicator on HabitCard (icon denoting "auto-completes from event/task") - 1h
- [ ] T1403 [US8] Write Jest tests for category filter, event-linking UI (8+ tests) - 2h
