...
> **DELIVERED 2026-09-10** — PR1 (`#131`, task tools) + PR2 (events, labels, resources).
> Corrections applied vs. the tasks as written below:
> - **Auth (T1601):** life-api has no refresh endpoint and issues no refresh token
>   (`RefreshTokenRequest` / `GenerateRefreshToken` are dead code). Implemented as
>   login → in-memory JWT cache → proactive re-login ~5 min before `exp` → 401-retry.
>   Authenticates as the **user's own account** (life-api has no cross-user scoping;
>   a service account would be an invisible third identity in a multi-user deployment).
> - **Priority:** string enum `Low|Medium|High|Critical`, not the numeric 1–5 in the spec.
> - **`complete_task`:** no dedicated endpoint — uses `PATCH /api/v1/tasks/{id}/status`.
> - **`search_tasks` (T1618):** deferred — no life-api search endpoint; a client-side
>   title filter would mislead.
> - **T1597:** no root tsconfig to extend — standalone emitting config.
> - **T1599:** `pnpm-workspace.yaml` already globs `apps/*` — no edit needed.
> - **Labels (T1622–T1624):** create body is `{ name, colourHex }` (`colourHex`, not
>   `colour`); `LabelDto` is `{ id, name, colourHex }`. No label update/delete tools in v1.
> - **Multi-backend:** built with a `createBackend` factory + registry so `finance-api`
>   / `finance_*` tools are a config bolt-on (per the vault Service Topology decision).
> - Tests: Jest + ts-jest + `axios-mock-adapter` (repo has no Vitest); 109 tests.

### Package Setup (Day 1)

- [x] T1596 [P] Initialise `apps/life-mcp/package.json` as `@life-manager/mcp`; configure `pnpm` workspace entry; add `@modelcontextprotocol/sdk`, `axios`, `zod`, `typescript`, `tsx` as dependencies — 1h
- [x] T1597 [P] Create `apps/life-mcp/tsconfig.json` extending root tsconfig; set `outDir: dist`, `module: Node16`, `target: ES2022` — 30m
- [x] T1598 Create `apps/life-mcp/src/index.ts` — entry point that instantiates `McpServer`, registers all tools and resources, and connects stdio transport — 1h
- [x] T1599 [P] Add `apps/life-mcp` to root `pnpm-workspace.yaml` and root `tsconfig.json` `references` array — 30m
- [x] T1600 [P] Add `build`, `start`, `dev` scripts to `apps/life-mcp/package.json`; add `mcp:build` and `mcp:start` to root `package.json` — 30m

### Auth Client (Day 1-2)

- [x] T1601 Create `apps/life-mcp/src/auth/life-manager-auth.ts` — `LifeManagerAuth` class that logs in with `LM_MCP_EMAIL` / `LM_MCP_PASSWORD` env vars on first call, stores JWT + refresh token in memory, and silently refreshes before expiry — 3h
- [x] T1602 Create `apps/life-mcp/src/api/life-manager-client.ts` — Axios instance pre-configured with `LM_API_BASE_URL`, `Authorization: Bearer <token>` header injected via interceptor using `LifeManagerAuth`, and `User-Agent: LifeManager-MCP/1.0` header — 2h
- [x] T1603 Add environment variable validation in `apps/life-mcp/src/config.ts` using Zod — fail fast at startup with a clear message if `LM_API_BASE_URL`, `LM_MCP_EMAIL`, or `LM_MCP_PASSWORD` are missing — 1h
- [x] T1604 Write unit tests for `LifeManagerAuth` (initial login, token refresh, failure handling — 8+ tests) in `apps/life-mcp/src/__tests__/auth.test.ts` — 2h

### Shared Types & Utilities (Day 2)

- [x] T1605 [P] Create `apps/life-mcp/src/types/task.ts` — TypeScript interfaces for `TaskResult`, `CreateTaskInput`, `UpdateTaskInput`, `SubtaskResult` matching API response shapes — 1h
- [x] T1606 [P] Create `apps/life-mcp/src/types/event.ts` — TypeScript interfaces for `EventResult`, `CreateEventInput`, `UpdateEventInput` — 1h
- [x] T1607 [P] Create `apps/life-mcp/src/types/label.ts` and `apps/life-mcp/src/types/stats.ts` — label and statistics response interfaces — 30m
- [x] T1608 [P] Create `apps/life-mcp/src/utils/format-date.ts` — helper that embeds today's date in tool description strings so the LLM has ambient date context — 30m
- [x] T1609 [P] Create `apps/life-mcp/src/utils/api-error.ts` — helper that maps API error responses to user-friendly MCP tool error strings — 1h

### Task API Wrapper (Day 3)

- [x] T1610 Create `apps/life-mcp/src/api/tasks-api.ts` — functions wrapping all task-related API calls: `listTasks`, `getTask`, `createTask`, `updateTask`, `deleteTask`, `searchTasks`, `completeTask`, `addSubtask`; each function typed with Zod-validated inputs — 4h
- [x] T1611 Write unit tests for `tasks-api.ts` using `axios-mock-adapter` (each function, success + error paths — 16+ tests) — 3h

### Task Tools (Days 3-5)

- [x] T1612 Implement `list_tasks` tool in `apps/life-mcp/src/tools/tasks/list-tasks.ts` — input schema: `status?`, `priority?`, `labelIds?`, `dueDate?`, `groupId?`, `limit?` (default 20, max 100); output: formatted task list — 2h
- [x] T1613 Implement `get_task` tool — input: `taskId`; output: full task detail including subtasks and labels — 1h
- [x] T1614 Implement `create_task` tool — input: `title` (required), `priority?` (1–5, default 3), `dueDate?` (ISO 8601), `notes?`, `labelIds?`, `groupId?`, `reminderAt?`; output: created task — 2h
- [x] T1615 Implement `update_task` tool — input: `taskId` + any subset of updatable fields; performs a partial update (PATCH semantics); output: updated task — 2h
- [x] T1616 Implement `complete_task` tool — input: `taskId`; sets status to `Completed`; idempotent (safe to call if already complete); output: confirmation message — 1h
- [x] T1617 Implement `delete_task` tool — input: `taskId`; output: confirmation; document in tool description that this is irreversible — 1h
- [ ] T1618 Implement `search_tasks` tool — input: `query` (min 2 chars), `limit?` (default 10); output: matching tasks list — 1h
- [x] T1619 Implement `add_subtask` tool — input: `parentTaskId`, `title`; output: created subtask — 1h
- [x] T1620 Register all task tools in `apps/life-mcp/src/tools/index.ts` — single function `registerTools(server: McpServer)` called from `index.ts` — 1h
- [x] T1621 Write Jest tests for each task tool (mocking `tasks-api.ts` — 24+ tests covering happy path + validation errors + API errors) in `apps/life-mcp/src/__tests__/tools/tasks/` — 4h

### Label API + Tools (Day 5)

- [x] T1622 [P] Create `apps/life-mcp/src/api/labels-api.ts` — `listLabels`, `createLabel` functions — 1h
- [x] T1623 [P] Implement `list_labels` and `create_label` tools in `apps/life-mcp/src/tools/labels/` — 1h
- [x] T1624 [P] Write tests for label tools (6+ tests) — 1h

### Phase 64 Integration & Smoke Test (Day 5)

- [x] T1625 Run the MCP server locally against the dev API; verify all Phase 64 tools work end-to-end via `npx @modelcontextprotocol/inspector` — 1h
...
> **PARTIALLY DELIVERED 2026-09-10** in PR2 — event tools (T1626–T1634), MCP resources
> (T1643–T1646, T1648), and the integration pass (T1649). Corrections:
> - **Events have NO recurrence** in life-api — `EventDto` has no RRULE field. Dropped
>   `deleteMode` / `updateMode` / `recurrenceRule` from T1626/T1630/T1631/T1632. Plain CRUD.
> - `create_event` body is `{ title, startDate, endDate, isAllDay?, description?, location?,
>   reminderMinutes?, groupId? }` (matches `CreateEventRequest`).
> - **Still deferred:** Stats tools (T1635–T1638), User/Settings tools (T1639–T1642),
>   the `life-manager://stats/week` resource (T1647), and all of Phase 66 (SSE transport).

### Event API Wrapper (Day 1)

- [x] T1626 Create `apps/life-mcp/src/api/events-api.ts` — `listEvents`, `getEvent`, `createEvent`, `updateEvent`, `deleteEvent` functions; `deleteEvent` accepts `deleteMode: 'single' | 'following' | 'all'` for recurring events — 3h
- [x] T1627 Write unit tests for `events-api.ts` (12+ tests) — 2h

### Event Tools (Days 1-2)

- [x] T1628 Implement `list_events` tool — input: `startDate`, `endDate`; output: formatted event list with recurrence notes — 2h
- [x] T1629 Implement `get_event` tool — input: `eventId`; output: full event detail — 1h
- [x] T1630 Implement `create_event` tool — input: `title`, `startDate`, `endDate`, `allDay?`, `notes?`, `recurrenceRule?` (RRULE string); add RRULE format examples to tool description — 2h
- [x] T1631 Implement `update_event` tool — input: `eventId` + updatable fields + optional `updateMode` for recurring events — 2h
- [x] T1632 Implement `delete_event` tool — input: `eventId`, `deleteMode`; document recurrence modes in tool description — 1h
- [x] T1633 Register event tools in `tools/index.ts` — 30m
- [x] T1634 Write tests for event tools (20+ tests) — 3h
...
- [x] T1643 Create `apps/life-mcp/src/resources/index.ts` — `registerResources(server: McpServer)` called from `index.ts` — 30m
- [x] T1644 Implement `life-manager://tasks/today` resource — calls `listTasks` filtered to today's due date; returns formatted markdown task list — 2h
- [x] T1645 Implement `life-manager://tasks/overdue` resource — calls `listTasks` with overdue filter; returns formatted list with days overdue — 2h
- [x] T1646 Implement `life-manager://events/upcoming` resource — calls `listEvents` for next 7 days; returns day-by-day formatted schedule — 2h
- [ ] T1647 Implement `life-manager://stats/week` resource — calls `getWeeklyStats`; returns markdown summary paragraph — 1h
- [x] T1648 Write tests for all resources (10+ tests) — 2h

### Phase 65 Integration Test (Day 5)

- [x] T1649 End-to-end test: launch MCP server against dev API and verify all Phase 65 tools and resources via MCP inspector — 1h
